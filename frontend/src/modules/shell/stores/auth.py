...
def is_loading(session: rio.Session) -> bool:
    try:
        return session[AuthSessionState].is_loading
    except KeyError:
        return False


def set_loading(session: rio.Session, loading: bool) -> None:
    session.attach(AuthSessionState(is_loading=loading))
