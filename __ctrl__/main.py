...
from utils.logs.cli import build_logs_subparser
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="fast-rio-ctrl",
        description=(
            "Single CLI for fast-rio: local dev, tests, prod helpers, and SSH VM ops."
        ),
        ...
        epilog=(
            ...
            "\n"
            "SSH (VM):\n"
            "  fast-rio-ctrl.bat setup | clone | env | start | stop | update\n"
            "  fast-rio-ctrl.bat backup-acme | status | connect"
        ),
    )
    ...
    p = sub.add_parser(
        "setup-local",
        help="Create/refresh project .venv + pip install -r requirements.txt",
    )
    p.add_argument(
        "--force",
        action="store_true",
        help="Delete .venv and recreate before installing",
    )
...
def _repl() -> int:
    parser = build_parser()
    echo("fast-rio-ctrl> (setup-local | dev run all | test all | list | quit)")
    while True:
        try:
