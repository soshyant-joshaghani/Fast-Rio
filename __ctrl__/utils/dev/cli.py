...
FULL_INFRA_SERVICES = ("db", "redis", "adminer")
SLIM_INFRA_SERVICES = ("db", "adminer")

# (window_title, port) — host processes; port None = no listener (e.g. ARQ worker)
APP_PROCESSES = (
    ("fast-rio-backend", 8000),
    ...
    if check and proc.returncode != 0:
        ...
        if "port is already allocated" in combined.lower() or "address already in use" in combined.lower():
            print(
                ...
                "  Apps still work directly at http://localhost:5000 and http://localhost:8000/docs\n"
...
def _setup_local(*, force_install: bool = False) -> int:
    """Ensure project .venv + requirements (no npm)."""
    _ensure_env()
    ...
    # Always ensure deps when marker missing, venv was just created, or forced.
    marker = PROJECT / ".venv" / ".fast-rio-deps-ok"
    need_install = created or not marker.is_file() or force_install
    if need_install:
...
def _start_infra(*, slim: bool = False) -> int:
    ...
    if not slim:
        print("[fast-rio] Waiting for Redis...")
        if _wait_redis() != 0:
            return 1
...
def _spawn_apps_windows(*, slim: bool = False) -> int:
    ...
    if wait_ports:
        print("  waiting for app ports :8000, :5000...")
...
def _start_apps(*, slim: bool = False) -> int:
    ...
    profile = "slim" if slim else "full"
    print(f"[fast-rio] Starting apps ({profile} runtime)...")
    if sys.platform == "win32":
    ...
    print()
    print(f"Dev stack ready ({profile} runtime, hot reload on save):")
    ...
    print()
    print("Direct: http://localhost:5000  http://localhost:8000/docs")
    if slim:
...
def _stop_apps() -> int:
    print("[fast-rio] Stopping host apps (backend / worker / frontend)...")
    killed: set[int] = set()
    for _title, port in APP_PROCESSES:
        if port is None:
            continue
        if sys.platform == "win32":
    ...
    if sys.platform == "win32":
        for title, _port in APP_PROCESSES:
            subprocess.run(
...
def _dev_help(args: argparse.Namespace) -> int:
    ...
    print(
        ...
        "\n"
        "run/start: compose up + host uvicorn/arq/rio\n"
        "  full (default): DB + Redis + ARQ worker + apps\n"
...
def cmd_setup_local(args: argparse.Namespace) -> int:
    """Top-level: create/refresh project .venv + requirements."""
    if getattr(args, "force", False):
        venv = PROJECT / ".venv"
        if venv.is_dir():
            print("[fast-rio] Removing existing .venv (--force)...")
            shutil.rmtree(venv)
    # Explicit command always refreshes pip deps (like the old setup-local scripts).
